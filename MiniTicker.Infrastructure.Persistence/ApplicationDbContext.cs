...
namespace MiniTicker.Infrastructure.Persistence
{
      ...
      public class ApplicationDbContext : DbContext
      {
            ...
            protected override void OnModelCreating(ModelBuilder modelBuilder)
            {
                  ...
                  ConfigureSystemEvent(modelBuilder);
          ...
          CancellationToken cancellationToken = default)
            ...
            private static void ConfigureArea(ModelBuilder builder)
            {
                  ...
                  entity.HasOne(e => e.Responsable)
                            ...
                            .OnDelete(DeleteBehavior.SetNull) // Si se borra el usuario, el área queda sin responsable
                            .IsRequired(false);
            ...
            private static void ConfigureUsuario(ModelBuilder builder)
            {
                  ...
                  entity.HasOne(u => u.Area)            // El usuario tiene un área
                .WithMany()                      // Un área puede tener muchos usuarios
                .HasForeignKey(u => u.AreaId)    // La clave foránea es AreaId
                .OnDelete(DeleteBehavior.Restrict);
                  //entity.Property<DateTime>("FechaCreacion").IsRequired();
                  //entity.Property<DateTime?>("FechaModificacion");
            }
            ...
            private static void ConfigureSystemEvent(ModelBuilder builder)
            {
                  var entity = builder.Entity<SystemEvent>();
                  entity.ToTable("SystemEvents"); // Nombre de la tabla en SQL

                  entity.HasKey(e => e.Id);

                  // Guardar el Enum como String para que sea legible en SQL (Login, Logout, etc.)
                  ...
                  entity.Property(e => e.Detalles)
                        .HasMaxLength(500); // Límite razonable para los mensajes

                  entity.Property(e => e.Fecha)
                        .IsRequired();

                  // Relación con Usuario (Si se borra el usuario, mantenemos el historial o lo borramos?)
                  // Generalmente para auditoría se prefiere Restrict o SetNull, pero aquí Restrict está bien.
                  entity.HasOne(e => e.Usuario)
                        .WithMany()
                        .HasForeignKey(e => e.UsuarioId)
                        .OnDelete(DeleteBehavior.Cascade) // O Restrict, depende de tu regla de negocio
                        .IsRequired();
            }
            #endregion
      }
